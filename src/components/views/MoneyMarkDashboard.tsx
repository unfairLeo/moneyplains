...
import { Home, BarChart3, Target, User, TrendingUp, Shield, Coins, PiggyBank, Landmark, Zap, Download } from "lucide-react";
  ...
  return (
    <div className="relative min-h-screen bg-[#050508] text-white overflow-hidden">
      <div className="max-w-[400px] mx-auto relative min-h-screen flex flex-col">
        ...
        {/* ── Quick Stats ── */}
        <motion.div className="px-6 pb-28 grid grid-cols-2 gap-3" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6, duration: 0.5 }}>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`relative flex flex-col items-center gap-1 px-4 py-2 rounded-xl transition-colors ${isActive ? "text-cyan-400" : "text-white/30"}`}
                    whileTap={{ scale: 0.9 }}
                  >
                    {isActive && (
                      <motion.div layoutId="navGlow" className="absolute inset-0 rounded-xl bg-cyan-500/10 border border-cyan-500/20" transition={{ type: "spring", stiffness: 400, damping: 30 }} />
                    )}
                    <Icon className="w-5 h-5 relative z-10" strokeWidth={1.5} />
                    <span className="text-[10px] font-medium relative z-10">{item.label}</span>
                    {isActive && (
                      <motion.div className="absolute -top-1 w-1 h-1 rounded-full bg-cyan-400" layoutId="navDot" style={{ boxShadow: "0 0 8px hsl(180 84% 45% / 0.8)" }} />
                    )}
                  </motion.button>
                );
              })}
